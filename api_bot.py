...
def generate_corpus(al_p_elements):
  corpus = ""
  for p_element in al_p_elements:
    corpus+= '\n' + ''.join(p_element.findAll(text=True))
  return corpus

#preprocessing (lemmantization, removing punctuation).
...
def get_bot_response(user_input):
  ...
  try:
    #use the google search api to fetch top 3 results.
    google_search_results = list(search(user_input, stop=3, pause=1))

    #use the request api to fetch the top result webpage.
    ...
    #extract all <p> elements from the webpage soup object.
    ...
    #Tokenisation.
    sentence_tokens = nltk.snet_tokenize(google_search_corpus) #converts raw text to list of sentences.
    
    #Calculate TFIDF matrix.
    sentence_tokens.append(user_input)
    tfidf_vectorizer = TfidVectorizer(tokenizer=LemNormalize, stop_words='english')
